...
// whether this transform will snap to recorded locations, or left accessible for your own handling
				...
				for( int frame = lastFrame + 1; frame <= currentFrame; frame++ )
                {
					...
					if ( snapToSyncedLocation )	// custom code more effective, but this is enough sometimes
					{
                        ...
                        if (!string.IsNullOrEmpty(posString))
                        {
                            ...
                            //Debug.Log("parseString" + ParseVector3(posString));
                            //transform.position = Vector3.MoveTowards(transform.position, ParseVector3(posString), Time.deltaTime * 1000);
                            // original code
	...
	{
        Debug.Log("GetProperty currentFrame" + currentFrame + "");
	...
	public static Vector3 ParseVector3( string vectorString )
	{
		...
		float x,y,z;
		if( splitVecString.Length == 6 && float.TryParse ( splitVecString[0], out x ) && float.TryParse ( splitVecString[2], out y ) && float.TryParse ( splitVecString[4], out z))
        {
            //Debug.Log("ParseVector3 sucess" + x + "," + y+ "," + z);
            return new UnityEngine.Vector3(x, y, z);
        }
        //Debug.Log("ParseVector3 fails");		
		return Vector3.zero;
	}
}
